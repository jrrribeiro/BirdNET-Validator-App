...
def test_bucket_health_inspector_validates_manifest_snapshot_and_event_layout() -> None:
    api = FakeBucketApi(
        {
            "metadata/project.json": json.dumps(
                {
                    "project_slug": "project-a",
                    "dataset_repo_id": "owner/audio",
                    "validation_backend": "hf_bucket",
                    "bucket_id": "owner/audio_validation_state",
                }
            ),
            "snapshots/current.json": json.dumps(
                {
                    "project_slug": "project-a",
                    "items": {
                        "audio-a-0000000001": {"status": "positive", "version": 1},
                    },
                }
            ),
            ...
            "archives/events/20260523.jsonl": json.dumps(
                {
                    "event_id": "archive-a",
                    "project_slug": "project-a",
    ...
    result = HfBucketHealthInspector(api=api).inspect(
        project_slug="project-a",
        dataset_repo_id="owner/audio",
        bucket_id="owner/audio_validation_state",
        token="hf_admin",
    ...
    assert result.snapshot_item_count == 1
    assert result.active_event_count == 1
    assert result.archive_file_count == 1
    assert result.latest_event_at == "2026-05-24T12:00:00+00:00"


def test_bucket_health_inspector_rejects_manifest_for_another_project() -> None:
    api = FakeBucketApi(
        {
            "metadata/project.json": json.dumps(
                {
    ...
    with pytest.raises(HfBucketValidationError, match="does not match"):
        HfBucketHealthInspector(api=api).inspect(
            project_slug="project-a",
            dataset_repo_id="owner/audio",
            bucket_id="owner/audio_validation_state",
            token="hf_admin",
...
def test_bucket_health_inspector_rejects_invalid_snapshot() -> None:
    api = FakeBucketApi(
        {
            "metadata/project.json": json.dumps(
                {
            ...
            "snapshots/current.json": json.dumps({"project_slug": "another-project", "items": {}}),
        }
    )

    with pytest.raises(HfBucketValidationError, match="snapshot"):
        HfBucketHealthInspector(api=api).inspect(
            project_slug="project-a",
            dataset_repo_id="owner/audio",
            bucket_id="owner/audio_validation_state",
...
def test_bucket_health_inspector_rejects_snapshot_missing_latest_archived_event() -> None:
    api = FakeBucketApi(
        {
            "metadata/project.json": json.dumps(
                {
            ...
            "snapshots/current.json": json.dumps({"project_slug": "project-a", "items": {}}),
            "archives/events/20260524.jsonl": json.dumps(
                {
                    "event_id": "archive-a",
                    "project_slug": "project-a",
                    "detection_key": "audio-a-0000000001",
    ...
    with pytest.raises(HfBucketValidationError, match="latest audit event"):
        HfBucketHealthInspector(api=api).inspect(
            project_slug="project-a",
            dataset_repo_id="owner/audio",
            bucket_id="owner/audio_validation_state",
